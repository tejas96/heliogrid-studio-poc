...
export interface BomContext {
  ...
  stoneRoofIdList: string[];
  /** how many stone roofs — the per-ROOF beam-locating and slab survey */
  stoneRoofCount: number;
...
export function buildContext(project: Project): BomContext | null {
  ...
  // An AC sheet is its OWN bucket, never the metal-shed one and never the
  // flat-RCC remainder. A metal shed's mini-rail-and-clamp price buys the wrong
  // fixing on asbestos cement — a shed takes a screw into a structural sheet,
  // an AC roof takes a hook bolt around the purlin — and the remainder bucket
  // would have quoted ballasted 10° tilt legs on a pitched fragile roof, which
  // is both the wrong money and a method nobody could carry out.
  const acRoofIds = new Set(
    project.roofs.filter((r) => r.roofType === 'ac_sheet').map((r) => r.id),
  );
  const acPanels = panels.filter(
  ...
  // A STONE SLAB is a flat deck to look at and a spanning plate to build on, so
  // it too is its own bucket. The flat-RCC remainder buys a table founded on a
  // cast pedestal — a point load on a 30 mm slab, which cracks it — and prices
  // a chemical anchor into concrete where what goes in is a bracket clamped to
  // the joist below, through a joint that then has to be re-pointed.
  const stoneRoofIds = new Set(
    project.roofs.filter((r) => r.roofType === 'stone_slab').map((r) => r.id),
  );
  const stonePanels = panels.filter(
  ...
  // quoted twice — once as themselves and once as elevated RCC.
  ...
  return {
    ...
    stoneRoofIdList: stonePanels.map((p) => p.roofId),
    stoneRoofCount: stoneRoofIds.size,
