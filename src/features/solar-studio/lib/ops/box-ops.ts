...
import type { ElectricalBox } from '../../types';
...
export const boxPlace = defineOp<{ kind: ElectricalBox['kind']; roofId: string; edgeIndex: number; t: number; heightM: number }>({
  id: 'box.place',
  layer: 'electrical',
...
export const boxMove = defineOp<{ id: string; roofId: string; edgeIndex: number; t: number; heightM?: number }>({
  id: 'box.move',
  layer: 'electrical',
  label: () => 'Move box',
