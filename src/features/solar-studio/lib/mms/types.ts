...
export type MountStrategy = 'rcc_fixed' | 'rcc_ballast' | 'rcc_anchor' | 'low_height' | 'elevated' | 'high_height' | 'east_west' | 'south_facing' | 'adjustable' | 'obstacle_clearance' | 'trapezoidal' | 'corrugated' | 'standing_seam' | 'clamp_mounted' | 'rail_mounted' | 'purlin_mounted' | 'rafter_mounted' | 'direct_sheet' | 'industrial_custom' | 'hook_bolt' | 'ac_spreader' | 'membrane_ballast' | 'aero_tray' | 'stone_beam_clamp' | 'stone_spread_ballast' | 'shed_seasonal' | 'carport_cantilever' | 'carport_portal' | 'carport_butterfly' | 'float_pontoon' | 'float_raft' | 'facade_rail' | 'facade_spandrel' | 'roof_hook' | 'adjustable_hook' | 'ground_pile' | 'ground_pedestal' | 'ground_ballast' | 'ground_seasonal' | 'ground_tracker' | 'flush' | 'custom';
